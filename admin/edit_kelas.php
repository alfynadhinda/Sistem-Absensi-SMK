<?php 
    ...
    $kod_kelas = $_GET['kode_kelas'];
    //Query data guru berdasarkan id
    $data_kelas = query("SELECT * FROM siarota_kelas WHERE kd_kelas = '$kod_kelas'")[0];
    ...
    <main class="mt-5 pt-3">
      <div class="container-fluid">
        <div class="row">
          ...
          <form action="log_edit_kelas.php" method="POST">
            <div class="mb-1">
               <label for="kode_kelas" class="form-label mb-1 mt-1 fw-bold fs-6">Kode Kelas</label>
               <input type="text" class="form-control" id="kode_kelas" name="kode_kelas" aria-describedby="kode_kelas" readonly value="<?= $data_kelas['kd_kelas']?>">
            </div>
            <div class="mb-1">
              <label for="nama_kelas" class="form-label fw-bold fs-6">Nama Kelas</label>
              <input type="text" class="form-control" id="nama_kelas" name="nama_kelas" aria-describedby="nama_kelas" value="<?= $data_kelas['nama_kelas']?>">
            </div>
            <div class="mb-1">
              <label for="guru_kelas" class="form-label fw-bold">Wali Kelas</label>
              <select id="guru_kelas" name="guru_kelas" class="form-select" aria-label="Default select example">
                <option selected value="<?= $data_kelas['wali_kelas']?>"><?= $data_kelas['wali_kelas']?></option>
                <?php
                    foreach ($guru as $row) :
                ?>
                <option value="<?= $row['nik_guru']?>"><?= $row['nik_guru']?> - <?=$row['nama_guru']?></option>
                <?php
                  endforeach
                ?>
              </select>
