<?php 
    ...
    $data_siswa = query("SELECT * FROM siarota_siswa WHERE nis_siswa = '$nis_siswa'")[0];
...
<body>
    ...
    <main class="mt-5 pt-3">
      <div class="container-fluid">
        ...
        <!-- Tabel Guru -->
        <form action="log_edit_murid.php" method="POST">
            <div class="mb-1">
            ...
            <div class="mb-1">
              <label for="alamat_siswa" class="form-label fw-bold fs-6">Alamat Siswa</label>
            ...
            <div class="mb-1">
              ...
              <input type="date" class="form-control" id="lahir_siswa" name="lahir_siswa" aria-describedby="lahir_siswa" value="<?= $data_siswa['lahir_siswa']?>">
            </div>
            <div class="mb-1">
              <label for="kelamin_siswa" class="form-label fw-bold fs-6">Jenis Kelamin</label>
              <select id="kelamin_siswa" name="kelamin_siswa" class="form-select" aria-label="Default select example">
                  <option value="<?= $data_siswa['kelamin_siswa']?>"><?= $data_siswa['kelamin_siswa']?></option>
                  <option value="Laki-Laki">Laki-Laki</option>
                  <option value="Perempuan">Perempuan</option>
              </select>
            <div class="mb-1">
              <label for="pwd_siswa" class="form-label fw-bold fs-6">Password</label>
              <input type="password" class="form-control" id="pwd_siswa" name="pwd_siswa" value="<?= $data_siswa['pwd_siswa']?>">
            </div>
            <div class="mb-1">
              <label for="kelas" class="form-label fw-bold fs-6">Kelas Siswa</label>
              <select id="kelas" name="kelas" class="form-select" aria-label="Default select example">
                  <option value="<?= $data_siswa['kelas']?>"><?= $data_siswa['kelas']?></option>
                  <?php
                    foreach ($kelas as $row) :
                  ?>
                  <option value="<?= $row['kd_kelas']?>"><?= $row['kd_kelas']?> - <?=$row['nama_kelas']?></option>
                  <?php
                    endforeach
                  ?>
              </select>
